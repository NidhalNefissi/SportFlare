...
const GymDetailDialog = React.memo(({ gym, open, onOpenChange, onClassClick, onCheckInClick, onBookClassClick }: GymDetailDialogProps) => {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[700px] z-[1003]" onInteractOutside={(e) => {
                e.preventDefault();
            }}>
                <DialogHeader>
                        ...
                        <div>
                            ...
                            <div className="space-y-2 mt-2">
                                {mockClasses.filter(c => gym && c.gym.id === gym.id).map((classItem) => (
                                    <div key={classItem.id} className="border rounded-md p-2 cursor-pointer hover:shadow" onClick={() => {
                                        if (onClassClick) {
                                            onClassClick(classItem.id);
                                        } else {
                                            onOpenChange(false);
                                            setTimeout(() => navigate(`/classes/${classItem.id}`), 0);
                                        }
                                    }}>
                                        <div className="flex items-center justify-between">
                                            <div>
                                                <div className="font-medium">{classItem.title}</div>
                                                <div className="text-xs text-muted-foreground">{new Date(classItem.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} with {classItem.coach.name}</div>
                                            </div>
                ...
                <DialogFooter className="flex sm:justify-between gap-2">
                    ...
                    <div className="flex gap-2">
                        <Button variant="outline" onClick={() => {
                            if (onCheckInClick) { onCheckInClick(); } else { onOpenChange(false); setTimeout(() => navigate(`/check-in/${gym.id}`), 0); }
                        }}>
                            <QrCode className="h-4 w-4" />
                            Check In
                        </Button>
                        <Button onClick={() => {
                            if (onBookClassClick) { onBookClassClick(); } else { onOpenChange(false); setTimeout(() => navigate(`/create-class?gymId=${gym.id}`), 0); }
                        }}>Book a Class</Button>
                    </div>
                </DialogFooter>
            </DialogContent>
