...
export const SubscriptionPlans: React.FC<SubscriptionPlansProps> = ({ onPlanSelected }) => {
  ...
  const [selectedPlan, setSelectedPlan] = useState<string | null>(user?.subscription?.planTier?.toLowerCase() || null);
  ...
  const calculateTotal = (monthlyPrice: number) => {
    // Apply 16% discount for annual plan (12 months)
    const months = selectedDuration === 12 ? 10 : selectedDuration; // 2 months free for annual
  ...
  const handleSelectPlan = (planId: string) => {
    ...
    if (onPlanSelected) {
      ...
      if (plan) {
        onPlanSelected(planId, selectedDuration, calculateTotal(plan.price));
      }
    }
  };
  ...
  const handlePaymentSuccess = (paymentMethod?: 'card' | 'payAtGym') => {
    ...
    if (selectedPlan) {
      // Update user's subscription in the context
      if (user) {
        const endDate = new Date();
        endDate.setMonth(endDate.getMonth() + selectedDuration);
        
        const subscriptionUpdate = {
          id: user.subscription?.id || `sub_${Date.now()}`,
          planTier: selectedPlan.toUpperCase() as 'BASIC' | 'PLUS' | 'PREMIUM',
          startDate: new Date().toISOString(),
          endDate: endDate.toISOString(),
          autoRenew: true,
          status: paymentMethod === 'payAtGym' ? 'pending' : 'active',
          paymentMethod,
          ...(user.subscription || {}) // Preserve other subscription properties
        };
      ...
      toast({
        title: paymentMethod === 'payAtGym' ? 'Payment Pending' : 'Subscription Updated',
        description: paymentMethod === 'payAtGym'
          ? `Your payment is pending. Please complete your payment at the gym counter.`
...
<PaymentModal
        ...
        amount={selectedPlan ? calculateTotal(plans.find(p => p.id === selectedPlan)?.price || 0) : 0}
        itemName={selectedPlan ? `${selectedPlan.charAt(0).toUpperCase() + selectedPlan.slice(1)} Plan (${selectedDuration} ${selectedDuration === 1 ? 'Month' : 'Months'})` : ''}
        type="subscription"
      />
    </div>
  );
